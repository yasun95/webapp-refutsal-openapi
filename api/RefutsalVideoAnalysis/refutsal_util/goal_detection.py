...
from refutsal_util.cam_csv_reader import CamCsvReader
...
class GoalDetector(FilePathCollector):
    def __init__ (self, cam_setting_dir, videodir):
    ...
    def setCamSettingDir(self,path):
        self.cam_setting_dir = path
    ...
    def setVideo(self, filename=None, filenum=None, camnum =1, resize=(None, None)):
        ...
        self.goalpoints = self.cam_setting.getCamSetting(self.camnum).goal_pos
    ...
    def __calSettingRatio(self):
        ...
        setting_frame = self.cam_setting.getCamSetting(self.camnum).frame_size
        self.setting_frame_w =setting_frame[0][0]
        self.setting_frame_h =setting_frame[0][1]

        #print("cam setting size", setting_frame[0][0], "*",setting_frame[0][1])
        self.csv_camsetting_size = setting_frame[0]

        if self.video_h/setting_frame[0][1] == self.video_w/setting_frame[0][0]:
            self.setting_ratio = self.video_h/setting_frame[0][1]
    ...
    def serchVideo(self, imgshow=False):
        path = os.path.join(self.videodir, self.videoname)
    ...
    def remapFrame(self, ignore_time):
        ...
        if len(self.goal_detect_frame):
            ...
            for i, time in enumerate(self.goal_detect_frame):
                sec= int(time/29.7%60)
                min= int(time/29.7//60)
    ...
    def printChart(self):
        ...
        print(cht_1.format("csv standard size",str(int(self.csv_camsetting_size[0]))+"*"+str(int(self.csv_camsetting_size[1]))))
        print(cht_1.format("resize ratio",self.setting_ratio))
        print(cht_1.format("video frame count", str(self.videolength)+" frame"))

        sec = self.videolength/self.videofps
        hour = int(sec//3600)
